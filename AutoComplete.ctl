...
Attribute ExitFocus.VB_Description = "Occurs when focus leaves the control.  The control itself could be losing focus, or a constituent control could be losing focus."
...
'Const m_def_SuggestedHeight = 0
Const m_def_SearchTable = "abbrev"
Const m_def_SearchField = "foodname"
'Property Variables:
Dim ShiftPressed As Boolean
Dim m_SelectedID As Long
Dim m_SuggestedHeight As Single
Dim mInternetSearch As Boolean
'Dim m_SuggestedHeight As Single
...
'Set OldLen as the sTextbox lenght
   ...
   Query = "SELECT * FROM " & m_SearchTable & " WHERE (" & Req & ")" & _
   ...
   If Not sTemp.RecordCount = 0 Then
        ...
        Else
            ...
            If mInternetSearch Then
                LSuggest.AddItem "Internet Search"
 ...
 '       LSuggest.Height = UserControl.TextHeight("^~$_)") + ScaleX(2, vbPixels, UserControl.ScaleMode)
  '      UserControl.Height = LSuggest.Top + UserControl.TextHeight("^~$_)") + ScaleX(4, vbPixels, UserControl.ScaleMode)
...
On Error Resume Next
  ...
  For i = 0 To LSuggest.ListCount - 1
   If LSuggest.Selected(i) Then
     SelectedEvent = 1
     If LCase$(LSuggest.List(i)) = "Internet Search" Then
       If LCase$(TData.Text) <> "Internet Search" Then
        Text = TData.Text
       Else
        Text = ""
       End If
...
On Error Resume Next
   If KeyAscii = 9 Or KeyAscii = 13 Then
    If LCase$(TData.Text) = LCase$(LSuggest.List(0)) Then TData.Text = LSuggest.List(0)
...
On Error Resume Next
    ...
    LSuggest.Height = UserControl.Height - LSuggest.Top
    RaiseEvent Resize
End Sub
...
On Error Resume Next
    ...
    Dim i As Long
    For i = 0 To LSuggest.ListCount - 1
      If LCase$(LSuggest.List(i)) = LCase$(New_Text) Then
         m_SelectedID = LSuggest.ItemData(i)
      End If
    Next
    If m_SelectedID = -777 Then m_SelectedID = 0
    If Not M_ListBox Is Nothing Then Call M_ListBox.LoadListBox(m_SelectedID)
...
Public Property Get InternetSearch() As Boolean
On Error Resume Next
    InternetSearch = mInternetSearch
End Property

Public Property Let InternetSearch(ByVal New_IS As Boolean)
On Error Resume Next
    mInternetSearch = New_IS
    PropertyChanged "InternetSearch"
End Property
